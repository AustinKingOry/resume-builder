  ...
  export async function generateFeedbackFromAnalysis(
    ...
    try {  
      ...
      if (logoBase64) {      
        ...
        // const imgRun = Media.addImage(
        // doc,
        // imgBuffer,
        // logoWidthPx,
        // logoHeightPx ?? Math.round((logoWidthPx * 40) / 120)
        // );
      ...
      const metadataTable = new Table({
        ...
        rows: [
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "File name" })] }),
              new TableCell({ children: [new Paragraph(sanitize(meta.fileName) || "—")] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "File size (bytes)" })] }),
              new TableCell({ children: [new Paragraph(fmtNum(meta.fileSize))] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "File type" })] }),
              new TableCell({ children: [new Paragraph(sanitize(meta.fileType) || "—")] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Page count" })] }),
          ...
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Processing time (ms)" })] }),
              new TableCell({ children: [new Paragraph(fmtNum(analysis.processingTime))] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Created at" })] }),
              new TableCell({ children: [new Paragraph(sanitize(analysis.createdAt))] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Complete" })] }),
              new TableCell({ children: [new Paragraph(analysis.isComplete ? "Yes" : "No")] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Finish reason" })] }),
              new TableCell({ children: [new Paragraph(sanitize(analysis.finishReason) || "—")] }),
            ],
          }),
          new TableRow({
            children: [
              new TableCell({ children: [new Paragraph({ text: "Usage (tokens)" })] }),
              new TableCell({
                children: [
                  new Paragraph(
                    usage
