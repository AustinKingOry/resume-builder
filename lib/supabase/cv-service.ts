...
export class SupabaseCVService {
  ...
  async getUserCVUploads(userId: string, limit = 10): Promise<CVUpload[]> {
    const { data, error } = await supabase
      .from("cv_uploads")
      .select("*")
      .eq("user_id", userId)
  ...
  async getUserRoastResponses(userId: string, limit = 10): Promise<(RoastResponse & { cv_upload: CVUpload })[]> {
    const { data, error } = await supabase
      .from("roast_responses")
      .select(`
        *,
  ...
  async getRoastResponseById(id: string): Promise<(RoastResponse & { cv_upload: CVUpload }) | null> {
    const { data, error } = await supabase
      .from("roast_responses")
      .select(`
        *,
  ...
  async getFeedbackForRoast(roastResponseId: string): Promise<Record<number, "up" | "down">> {
    const { data, error } = await supabase
      .from("user_feedback")
  ...
  subscribeToRoastResponses(userId: string, callback: (response: RoastResponse) => void) {
    return supabase
